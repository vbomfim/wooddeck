...
**Owner**: @vbomfim
**Last updated**: 2026-07-05 (feat/blocking-between-joists — Issue #72 blocking-between-joists (IRC R502.7 / R502.7.1) — the dormant `blocking` `MemberKind` and `BlockingLayer` are RE-ATTACHED. A new pure domain helper `layoutBlockingBetweenJoists` emits SOLID lumber noggins between adjacent joists at interior mid-span rows; ON-CENTER row pitch ≤ 8 ft with ≥ 1 mid-span row whenever there are ≥ 2 joists. The helper is called from BOTH the elevated pipeline (`computeLayout`, posts-on-footings AND deck-blocks variants) AND the floating pipeline (`computeFloatingLayout`, Method A AND Method B); the two pipelines derive the same joist x-centers and joist y-plane (`computeYStack{,Floating}.joistCenterY`) so blocking cannot drift from the joists it restrains. The `LayerVisibility.blocking` toggle, `LAYER_ITEMS` "Blocking" row, `DECK_LAYER_ORDER` (blocking sits between beams and joists in the physical stack), and `<BlockingLayer/>` mount in `DeckLayers.tsx` are all restored. All 20+ layout golden fixtures were regenerated — blocking members are appended to every elevated + floating fixture. The BOM's existing per-kind length rule (`derive-bom.ts:132` — blocking length = `size.x`, joist SKU pack) picks up blocking automatically; a computeLayout→deriveBom integration test proves it.)
**Issue tracker**: (populated when the epic issue is created — see Decomposition)
**Tickets**: Epic #1 · Stories #2–#16 (S1–S15) · #38 (S16) · **Epic 2 (Phase F): #39 (S17), #40 (S18), #41 (S19), #42 (S20), #43 (S21), #44 (S22), #45 (S23), #46 (S24), #47 (S25), #48 (S26), S27 (feat/joist-beam-connection PR #64) · #72 (feat/blocking-between-joists — dormant `blocking` `MemberKind` re-attached; IRC R502.7 lateral-restraint noggins between joists)** — see Decomposition

> **2026-07-05 amendment (feat/blocking-between-joists — issue #72; IRC R502.7 / R502.7.1):** The dormant `blocking` `MemberKind` — deferred by S26 FIX #6 as a placeholder for "blocking between joists" — is RE-ATTACHED. A new pure domain helper `layoutBlockingBetweenJoists({ joistXCenters, joistCenterY, joistThicknessMm, joistDepthMm, lengthMm, material })` emits SOLID lumber noggins between EVERY pair of adjacent joists at one or more interior mid-span row stations along +z. The row rule is `N = max(1, ceil(lengthMm / MAX_BLOCKING_SPACING_MM) - 1)` with `MAX_BLOCKING_SPACING_MM = 2438 mm (8 ft)`; rows are evenly spread `z_k = -L/2 + k × L/(N+1)` for `k = 1..N`, so a mid-span deck with N=1 gets a single row at z=0, and the floor of ≥ 1 interior row is triggered whenever there are ≥ 2 joists (per IRC R502.7.1 "at intervals not exceeding 8 feet"; the floor is a conservative DIY choice — the code prescribes bridging OR blocking for joist depth > 2×10 nominal, but blanket application to every joist depth is a safe superset). Fewer than 2 joists ⇒ zero blocking (no adjacent bay).
>
