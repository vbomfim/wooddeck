...
**Owner**: @vbomfim
**Last updated**: 2026-07-05 (fix/joists-on-blocks-flying — Method B block COLUMNS re-pinned to joist x-centers so every joist has a support column beneath it; the "regular grid on both axes" wording from S28 that decoupled columns from joists is superseded; `blockSpacingMm` now controls the ROW pitch only — the span-relevant along-joist support spacing)
...
- **FR-029** *(Floating-deck layout engine — parallel to the elevated engine; rescoped 2026-07-05 S26 review-gate)*: When `structure = 'floating'`, the layout engine MUST produce a `Layout` that dispatches on `design.floatingFraming` (see FR-033) and MUST honor `design.joist.spacingMm` (the pre-S26 layout ignored it — that is a fixed bug). The floating layout adds ONE new `MemberKind` (`block`) beyond the pre-amendment five (`joist`, `beam`, `post`, `footing`, `board`). The additional `blocking` `MemberKind` is RESERVED — no floating (or elevated) layout path emits `blocking` members in the current MVP; the kind is preserved for a future "blocking between joists" feature and the `BlockingLayer` component remains in the tree as a dormant `() => null`. The two active floating paths are:
  ...
  - **Method B — `'joists-on-blocks'`.** Beam-less framing. The layout MUST produce: (a) N joists running along the LENGTH axis at `design.joist.spacingMm` (same helper as Method A); (b) NO beam members; (c) a block grid whose COLUMNS are pinned to the joist x-centers (one block column per joist — every joist has a support column beneath it) and whose ROWS are placed along +z at pitch `foundation.blockSpacingMm`, with the outer rows anchored at the length axis ends (`computeAxisCenters(lengthMm, rows)`); row count = `max(2, ceil(lengthMm / spacingMm) + 1)` — the perimeter-two floor + a `ceil` step that guarantees adjacent-block ROW gap ≤ `spacingMm` (the span-relevant along-joist support pitch); total blocks = `numJoists × rows`; (d) decking directly on the joists. y-stack (bottom-up): block → joist → decking. **[amended 2026-07-05 — fix/joists-on-blocks-flying]** The S28 "regular grid on both axes / joists no longer sit exactly over a block column" model was superseded by UAT: decoupling block columns from joist x-centers caused most joists to sit over empty space between block columns and "fly" (float unsupported). A no-beam Method B deck is only physically sensible with a block DIRECTLY under EACH joist — the pre-S28 column-pinning is restored (`explicitColXCenters = joists.map(j => j.position.x)`) while the user-controllable ROW pitch that S28 added is retained (now the only `blockSpacingMm` seam — the along-joist support spacing, which is the span-relevant dimension anyway). See FR-035 for the full corrected model.
...
- **FR-032** *(Actionable "add support" remediation — supersedes S16 #38 Q1; rescoped 2026-07-05 S26 review-gate)*: The remediation-options set from FR-025 MUST include, when applicable, an `'add-support-row'` option that resolves an over-span warning by adding an intermediate row of foundation blocks (for `structure = 'floating'` under **Method B** — `floatingFraming = 'joists-on-blocks'`) or an intermediate beam (post-MVP, tracked separately for `structure = 'elevated'`). When applied, the option MUST update the design so `spanCheck` re-runs against the new support geometry; if the new geometry still over-spans, a new warning is emitted (the remediation may only REDUCE, not necessarily clear — matching FR-025's `wouldClear` discipline). The option is DISABLED with a reason when the current design cannot accept an intermediate support:
  ...
  - **Floating + Method B (`'joists-on-blocks'`)** — **[amended 2026-07-06 — S28 feat/block-spacing]** the option is ENABLED and shortens the joist support span by REDUCING `foundation.blockSpacingMm` (tighter grid → smaller block-row pitch along +z → shorter between-supports joist span). The proposed value is the LARGEST `blockSpacingMm` ≤ the current effective spacing that yields a row pitch ≤ `warning.allowableMm`, clamped to `[MIN_BLOCK_SPACING_MM, MAX_BLOCK_SPACING_MM]`; the patch dispatches `{ foundation: { blockSpacingMm: proposedSpacingMm } }` and is verified by the standard `verifyPatchClears` recompute. The remediation label is length-based ("Reduce block spacing to add support (X → Y)"). When the current effective spacing is already at `MIN_BLOCK_SPACING_MM` AND the warning still fires (only physically reachable for synthetically-weak joists), the option MUST be DISABLED with the reason "Block spacing is already at the minimum — reduce joist size or deck length." Pre-S28 this remediation wrote `blockRowsHint`, but S28's `blockSpacingMm` resolver ignores hints, so `blockRowsHint` is LEGACY-SUPERSEDED for Method B; the legacy hint-bump code path is retained in `apply-remediation.ts` only for backwards-compat with any pre-S28 remediation payloads that lack `proposedSpacingMm`.
...
- **FR-035** *(Method B block layout — columns pinned to joists, rows at user-selectable spacing; added 2026-07-06 S28 review-gate — feat/block-spacing PR #66; **corrected 2026-07-05 — fix/joists-on-blocks-flying**)*: For `structure === 'floating' && floatingFraming === 'joists-on-blocks'` (Method B), the block layout MUST place block COLUMNS at the joist x-centers (one block column per joist — `explicitColXCenters = joists.map(j => j.position.x)`; the block layer reads positions off the joist members so column x's are byte-identical to joist x's, no re-derivation drift) AND block ROWS along +z at user-selectable pitch `foundation.blockSpacingMm: Mm`, with the outer rows anchored at the length axis ends (`computeAxisCenters(lengthMm, rows)`). Row count MUST be `blockCountForAxis(lengthMm, effectiveSpacingMm) = max(2, ceil(lengthMm / effectiveSpacingMm) + 1)` — the perimeter-two floor + a `ceil` step that guarantees adjacent-block ROW gap ≤ `effectiveSpacingMm` (the along-joist support pitch, which is the span-relevant dimension). Total blocks = `numJoists × rows`. Every joist MUST have a block column beneath it (the "no flying joists" invariant — pinned by `floating-flying-joists.test.ts`). This SUPERSEDES the S28 "regular grid on both axes" model, which decoupled block columns from joist x-centers and produced a physical-plausibility regression (most joists sat over empty space between block columns and "flew" unsupported). `blockSpacingMm` is OPTIONAL on both block-carrying `FoundationSpec` variants (`deck-blocks` + `tuffblocks`); when ABSENT the layout MUST default to `DEFAULT_METHOD_B_BLOCK_SPACING_MM = 1220 mm (4 ft)` — a sensible ground-level DIY pitch chosen to keep the default joist span within IRC allowables for the MVP joist catalog. When PRESENT the value MUST be clamped to `[MIN_BLOCK_SPACING_MM = 300 mm, MAX_BLOCK_SPACING_MM = 2438.4 mm = 8 ft]` at BOTH the layout boundary AND the UI input boundary (so the stored value is always schema-legal — save/reload cannot trap the user in an invalid state). Total block count MUST be bounded by `MAX_METHOD_B_BLOCK_COUNT = 400` via DETERMINISTIC O(1) enforcement in `resolveMethodBGrid`: the cap reduces ROWS ONLY (columns are load-bearing and cannot be dropped). `maxRows = max(2, floor(MAX_METHOD_B_BLOCK_COUNT / numJoists))`; `rows = max(2, min(rowsReq, maxRows))`. Perimeter-two floor (`rows ≥ 2`) ALWAYS wins over the cap — a single support line under a rectangular deck is nonsensical. Degenerate corner: when `numJoists × 2 > MAX_METHOD_B_BLOCK_COUNT` (only reachable with pathological joist spacing on a near-max footprint), rows stay at 2 and the total is emitted directly, bounded by `MAX_DECK_DIMENSION_MM` (100 ft) × the joist-spacing physics (`validateJoistSpacing`) — no OOM. The postcondition assertion (`throw LayoutError` if `cols × rows > MAX && rows > 2`) MUST guard the return so any future arithmetic bug surfaces immediately rather than hiding behind a silently-too-dense grid; `rows === 2` is exempt (perimeter-two supersedes). No iteration cap — the enforcement is O(1). Defense-in-depth: `.deck` schema v2 `Dimensions3D.widthMm` / `lengthMm` MUST carry `maximum: 30480 mm (100 ft)` (a residential cap; the shared `Mm` `$def` allows 1,000,000 mm which is oversized for this app), AND `MAX_DECK_DIMENSION_MM` MUST be mirrored at BOTH the elevated `validateDesign` and the floating `validateFloatingDesign` trust boundaries so an in-memory patch / test-fixture path (no Ajv round-trip) hits the same guard. When `blockSpacingMm` is present the resolver PREFERS it over the legacy `blockRowsHint` (which remains in the schema for backwards-compat but is LEGACY-SUPERSEDED for Method B); `blockColsHint` is a NO-OP for Method B (columns are pinned to the joist count — the number of load-bearing supports per joist is a physical property, not a UI knob). Span-check for Method B is UNCHANGED in shape: `span-check.ts::deriveMethodBJoistSpanFromBlockGrid` derives the joist support span from the block GRID row pitch (max adjacent-z gap between block rows), so an `over-span-joist` warning fires when the user picks a `blockSpacingMm` too large for the joist material and clears when they pick one small enough. Every joist MUST be span-checked (post-fix every joist has its own block column, so a joist skipped by x-position was already forbidden — this pin is retained as a regression guard). The Method B `'add-support-row'` remediation from FR-032 MUST reduce `blockSpacingMm` (see FR-032 Method B bullet); the remediation caller passes `numJoists` (from `layoutFloatingJoists(design).length`) to `resolveMethodBGrid` so the reported `currentRows` reflects the cap-by-rows path. Persistence: `blockSpacingMm` is an OPTIONAL number in `.deck` schema v2 with `minimum: 300`, `maximum: 2438.4`, `additionalProperties: false` preserved. Missing on load → the layout applies the default at compute time; existing pre-S28 `.deck` files continue to load unchanged. The UI MUST expose a `BlockSpacingField` (LengthField idiom — imperial/metric via `formatLength` / `parseLength`) VISIBLE only when `structure === 'floating' && floatingFraming === 'joists-on-blocks'`; commits MUST clamp `[MIN, MAX]` BEFORE dispatch. The field's helper text MUST clarify the new semantics — the field controls the spacing between block ROWS ALONG each joist (bounds the joist span between supports), NOT the whole-grid pitch. `MIN_BLOCK_SPACING_MM` / `MAX_BLOCK_SPACING_MM` / `DEFAULT_METHOD_B_BLOCK_SPACING_MM` MUST be re-exported through the `state` barrel (the dep-cruiser `ui-no-domain-layout` rule forbids the UI from importing `src/domain/layout/**` directly). **Float-drift tolerance.** `computeAxisCenters` accumulates `-span/2 + i × step` (`step = span / (count − 1)`); IEEE-754 mul-add rounding can drive the LAST center a few femtometres OUTSIDE `±span/2` for fractional (schema-legal) footprints (e.g. `span=10205.753894382558, count=19 → last=5102.876947191281` vs bound `5102.876947191279`, Δ ≈ 1.8e-12 mm). `validateExplicitCenters` MUST accept centers within `±span/2 ± EPS_MM` where `EPS_MM = 1e-6 mm` (matching `validateJoistSpacing`'s tolerance in `layout-shared.ts`) so schema-legal fractional footprints do not crash on drift. The tolerance is a nanometre — a real callsite bug (millimetres out of range or worse — the S26 defense's target) is still rejected. **Regression pin.** Method A + elevated + posts-on-footings layouts MUST be BYTE-IDENTICAL to pre-fix (`blockSpacingMm` alters ONLY Method B; the validator tolerance changes no block positions); every pre-S28 golden fixture stays unchanged.
