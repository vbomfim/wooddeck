...
**Independent Test**: With the parameter panel, switching the construction MODEL from "Elevated" to "Floating" replaces the posts + drop-beams layout with a block-grid + beams-on-blocks + blocking layout within 500 ms; the BOM shows the block count separately and packs blocking offcuts into stock 2×8s. A mini worked example on the ~16′ × 14′ TuffBlock reference deck (S19 IRC-sparse blocking = 48″ per FR-029(c)): 3 beams → 2 beam-bays × 3 blocking pieces per bay = 6 blocking pieces of ~2 ft each, packed into <1 additional 2×8×16 stock board on top of the full-length beams. (Denser bracing would appear if a future story surfaces the blocking-spacing constant to the UI.)
...
- **FR-029** *(Floating-deck layout engine — parallel to the elevated engine)*: When `structure = 'floating'`, the layout engine MUST produce a `Layout` with two new `MemberKind` values in addition to the existing five (`joist`, `beam`, `post`, `footing`, `board`): `block` (an on-grade foundation block from the foundation-block catalog) and `blocking` (a short lumber offcut braced perpendicularly between beams for lateral rigidity). The floating-model layout MUST: (a) place a grid of blocks on grade under every beam-to-beam intersection, with the block-row spacing along the beam DERIVED from the beam's tributary-aware allowable span (`SpanTable.lookupBeamMaxSpan(beam.material, tributary = beam-to-beam +x delta, 2 plies)`) and capped at a practical DIY minimum (24″ = 610 mm for MVP — `BLOCK_ROW_MAX_SPACING_MM_PRACTICAL` in `floating-layout.ts`); the derived value is `min(practical, allowable)`, so a large-SKU beam does not spawn a sparse grid that looks wrong to a carpenter; (b) place full-length beams resting on the block grid along ONE deck axis (MVP hard-codes the axis to LENGTH — a `beamAxis` design parameter is deferred post-MVP and NOT modelled in `DeckDesign` today); (c) place blocking members between adjacent beams at IRC R502.7 48″ (1220 mm) sparse bracing spacing — a fixed value NOT UI-configurable in the MVP (user Q2, ticket §17 §14); a short deck whose beam-pair-gap is less than 1220 mm produces ZERO blocking pieces (physically correct); (d) place decking on top, oriented perpendicular to the beams by default. Posts and footings MUST NOT appear in a floating layout. Existing FR-002 (auto-layout) and FR-005 (individual-member geometry) apply uniformly to both models.
