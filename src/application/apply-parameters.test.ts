...
describe('applyParameters — beamConnection (S27)', () => {
  ...
  it('G4 (S27 review-response): beamConnection persists across structure + framing-method switches', () => {
    ...
    // NOTE (issue #77): flush + interior now RENDERS with segmented
    // joists (issue #75's FR-E throw was removed in #77). The 8 ft
    // length keeps `interiorRows === 0` so this test still exercises
    // the pre-#77 flush-2-beam (byte-identical) path — this test's
    // intent is that `beamConnection` PERSISTS across structure +
    // framing switches, which is independent of segmentation.
