...
**Owner**: @vbomfim
**Last updated**: 2026-07-05 (PR #73 review-gate follow-up — GPT-5.5 Code Review + QA Guardian confirmed fixes on the feat/blocking-between-joists PR: (1) `layoutBlockingBetweenJoists` now SKIPS degenerate zero-width bays where `clearGap ≤ CLEAR_GAP_EPS_MM = 1e-6` — the pre-fix helper emitted invisible `size.x == 0` members whenever `validateJoistSpacing` admitted `actualSpacing == joistThickness` (touching joists); (2) member-count-bound docstring added to `blocking-layout.ts` — bounds derived from the elevated / floating footprint cap (`MAX_DECK_DIMENSION_MM = 30480 mm`) plus the zero-width guard, so no arbitrary numeric cap is needed; (3) FR-036 + helper docstring no longer over-claim IRC R502.7 END restraint for floating Method B (`joists-on-blocks`), which has no rim beam — end restraint for Method B is now explicitly deferred to issue #74; (4) barrel `src/domain/layout/index.ts` re-exports `MAX_BLOCKING_SPACING_MM` from the ACTIVE `./blocking-layout` (= 2438 mm) and the DEAD legacy `1220 mm` constant of the same name was DELETED from `floating/floating-layout.ts` — eliminating the name / value collision; (5) the weak per-unique-length BOM assertion in `derive-bom.test.ts` was strengthened to a per-length COUNT assertion so silent drops of identical blocking pieces can no longer false-pass. QA's `blocking-integration.test.ts` (150-cell matrix) and `persistence/blocking-roundtrip.test.ts` are committed with the fixes.)
**Issue tracker**: (populated when the epic issue is created — see Decomposition)
**Tickets**: Epic #1 · Stories #2–#16 (S1–S15) · #38 (S16) · **Epic 2 (Phase F): #39 (S17), #40 (S18), #41 (S19), #42 (S20), #43 (S21), #44 (S22), #45 (S23), #46 (S24), #47 (S25), #48 (S26), S27 (feat/joist-beam-connection PR #64) · #72 (feat/blocking-between-joists — dormant `blocking` `MemberKind` re-attached; IRC R502.7.1 lateral-restraint noggins between joists — PR #73 review fixes: zero-width bay guard + Method B end-restraint scope clarification + barrel const collision resolved; end restraint for Method B deferred to #74)** — see Decomposition

> **2026-07-05 amendment (PR #73 review-gate fixes — GPT-5.5 Code Review Guardian HIGH #1, #2, #3, MEDIUM #4 + QA Guardian BOM strengthening; same PR / same worktree — applied on top of the feat/blocking-between-joists commit `08c6ac7`):** The review gate on PR #73 returned confirmed issues; each is applied in-PR:
>
> **HIGH #1 — Zero-width blocking guard.** `validateJoistSpacing` (`layout-shared.ts`) admits `actualSpacing == joistThickness` (adjacent joists touching face-to-face). At that boundary the CLEAR gap between joist faces is zero, and the pre-fix `layoutBlockingBetweenJoists` emitted a `size.x == 0` invisible mesh + a zero-length BOM cut — a lying UI. The helper now applies a `CLEAR_GAP_EPS_MM = 1e-6` guard (kept in strict lock-step with the upstream validator's `EPS_MM`) and SKIPS any bay whose clear gap ≤ EPS. Fail-safe skip, not a throw. Row loop continues so any OPEN bay in the same row still emits blocking normally. Unit tests added to `blocking-layout.test.ts`. QA's `[EDGE] documents…zero-width` test in `blocking-integration.test.ts` (which explicitly documented the pre-fix `size.x === 0` behavior as a known LOW-sev edge and invited a future guard) is UPDATED to expect the new skip-in-touching-bays behavior.
>
> **HIGH #2 — Member-count bounds documented; no arbitrary cap added.** GPT flagged the theoretical member count on a 100 ft deck at near-thickness spacing (~9,600 blocking members). The pathological case requires clear gap ≈ 0 → HIGH #1's guard SKIPS all those bays → ~0 blocking. At realistic joist spacing (12″ / 16″ / 24″ o.c.) a maximally-legal 100 × 100 ft deck emits ~600 to ~1200 blocking members — proportionate to a legally-huge deck. Adding an arbitrary numeric cap could SILENTLY drop rows on a legal large deck (silent under-bracing — worse than the DoS). Instead, a member-count-bounds docstring is added to `blocking-layout.ts` explaining the pipeline-imposed bounds (`MAX_DECK_DIMENSION_MM = 30480 mm` footprint cap × the zero-width guard). No LEGAL input has been found that produces > ~1200 blocking with positive clear gap; if one is ever discovered the docstring instructs the reviewer to escalate for a fail-loud vs. accept decision rather than silently cap.
>
> **HIGH #3 — Method B end-restraint over-claim corrected.** IRC R502.7 requires the joist ENDS to be restrained (rim / band / ledger). This is provided by the rim joist / ledger for elevated framing AND by the two rim beams for floating Method A, but is NOT provided for floating Method B (`joists-on-blocks`) which has no rim/band member at present. The prior FR-036 + `blocking-layout.ts` docstring implied end restraint was universally satisfied — TRUE for elevated + Method A, FALSE for Method B. Both the FR and the helper docstring are softened to say: this feature provides INTERIOR bridging / anti-rotation restraint per IRC R502.7.1 (≤ 8 ft o.c.) only; end restraint is provided separately by the rim members where present. For floating Method B, adding a joist-depth end band is deferred to a separate story tracked in **issue #74**. The interior-blocking behavior is UNCHANGED — only the claim is corrected.
>
> **MEDIUM #4 — Barrel constant collision resolved.** Two modules previously defined `MAX_BLOCKING_SPACING_MM` with DIFFERENT values: `blocking-layout.ts` = 2438 (the ACTIVE IRC R502.7.1 value) and `floating/floating-layout.ts` = 1220 (a DEAD pre-#72 legacy stub with no runtime consumer). The barrel `src/domain/layout/index.ts` accidentally re-exported the 1220 version. FIX: the barrel now re-exports the constant from `./blocking-layout` (= 2438); the dead `export const MAX_BLOCKING_SPACING_MM: Mm = 1220;` in `floating-layout.ts` is DELETED. A new smoke test `src/domain/layout/index.barrel.test.ts` pins the barrel value at 2438 so any future re-shuffle fails fast. Grep verified no other consumer depended on the 1220 value.
>
> **QA — BOM assertion strengthened.** The prior `derive-bom.test.ts` integration test iterated only DISTINCT blocking lengths ("each unique length appears ≥ once in the pack") — a false pass on evenly-spaced joists where every blocking piece has the same length (`distinctLengths.size === 1`). The strengthened test asserts per-length COUNT parity (`actual >= expected` for each length) so silent drops of identical pieces are caught. QA's `blocking-integration.test.ts` (150-cell matrix — 5 structures × 5 sizes × 3 spacings × 2 species + BOM per-member) and `persistence/blocking-roundtrip.test.ts` (serialize → deserialize → recompute determinism) are committed alongside the fixes and pass.

---

> **2026-07-05 amendment (feat/blocking-between-joists — issue #72; IRC R502.7 / R502.7.1):** The dormant `blocking` `MemberKind` — deferred by S26 FIX #6 as a placeholder for "blocking between joists" — is RE-ATTACHED. A new pure domain helper `layoutBlockingBetweenJoists({ joistXCenters, joistCenterY, joistThicknessMm, joistDepthMm, lengthMm, material })` emits SOLID lumber noggins between EVERY pair of adjacent joists at one or more interior mid-span row stations along +z. The row rule is `N = max(1, ceil(lengthMm / MAX_BLOCKING_SPACING_MM) - 1)` with `MAX_BLOCKING_SPACING_MM = 2438 mm (8 ft)`; rows are evenly spread `z_k = -L/2 + k × L/(N+1)` for `k = 1..N`, so a mid-span deck with N=1 gets a single row at z=0, and the floor of ≥ 1 interior row is triggered whenever there are ≥ 2 joists (per IRC R502.7.1 "at intervals not exceeding 8 feet"; the floor is a conservative DIY choice — the code prescribes bridging OR blocking for joist depth > 2×10 nominal, but blanket application to every joist depth is a safe superset). Fewer than 2 joists ⇒ zero blocking (no adjacent bay). Zero-width bays (touching joists) are also SKIPPED — see PR #73 review amendment above.
>
> Per-member geometry, in the joist plane (y = `computeYStack{,Floating}.joistCenterY`): `size.x` = clear gap between the two joist FACES (`xCenters[i+1] − xCenters[i] − joistThickness`), `size.y` = joist depth (full-depth solid blocking), `size.z` = joist thickness (blocking is the SAME 2× lumber as the joist, oriented with its thickness along +z); `position.x` = bay midpoint; `position.y` = joistCenterY; `position.z` = row station; `material` = the joist's lumber material (so blocking piggybacks on the joist BOM SKU and renders emerald via the existing `MATERIAL_KIND_COLORS.blocking`). Deterministic ids `blocking-r{row}-b{bay}`.
>
> The helper is called from BOTH pipelines: the elevated `computeLayout` (posts-on-footings AND deck-blocks variants) via `computeBlockingFromDesign(design)`, and the floating `computeFloatingLayout` (Method A + Method B) via `computeFloatingBlockingFromDesign(design)`. Both adapters derive x-centers from the SAME `computeJoistXCenters(widthMm, spacingMm, thicknessMm)` used by the joist layout, and derive y from the SAME y-stack helper the joists read, so blocking cannot drift from the joists it restrains.
>
> The dormant UI/scene infrastructure is fully re-attached: `LayerVisibility.blocking: boolean` returns (default ON — safety-relevant), `LAYER_ITEMS` gains a "Blocking" row between "Joists" and "Beams" (matches the physical-stack top-down toggle order), `DECK_LAYER_ORDER` inserts `'blocking'` between `'beams'` and `'joists'` (co-planar with joists, rendered before joists in the bottom-up scene-graph order), `DeckLayers.tsx` mounts `<BlockingLayer />` between `<BeamsLayer />` and `<JoistsLayer />`, and `BlockingLayer.tsx` replaces its dormant `return null` body with `<KindLayer kind="blocking" visibilityKey="blocking" />` (identical delegate signature to every sibling KindLayer).
>
> BOM: the existing per-kind length rule in `derive-bom.ts` already handled `blocking` (length = `size.x`, runs along +x — the S22 wiring survived deletion of the emitter). Blocking members from every design now fold into the joist SKU's FFD cut-list pack automatically; an integration test proves `deriveBom(computeLayout(defaultDesign))` includes blocking cuts.
>
> Domain boundary: the new helper is a PURE function with NO react/three imports; `scripts/boundary-selftest.mjs` stays 28/28. All ~20 layout golden fixtures were REGENERATED (blocking members now appended to every elevated + floating fixture) — the JSON diffs are expected and reviewed. Single-joist narrow decks (fewer than 2 joists) still emit zero blocking with no crash.
>
> Persistence: NO `.deck` schema change — blocking is a derived output of `computeLayout`, not a persisted design field. Every existing `.deck` file continues to load byte-identical; the SAVED design is unchanged and the RENDERED layout adds the code-mandated noggins on load. New FR-036 (below) codifies the behavior; FR-029's "blocking `MemberKind` is RESERVED" clause is SUPERSEDED — blocking is now emitted by every layout path.
...
> **2026-07-05 amendment (feat/block-count-per-joist SECOND review-gate follow-up — diff-review fixes on PR #71, same day):** The count-primary follow-up above went through a second diff-only review (Opus + GPT-5.5). Both approved for merge (no blockers), but two 🟡 MEDIUM findings + two 🔵 LOW cleanups on the just-touched files reinforced this PR's "no-lying UI" bar. The following are applied in-PR:
>
> - **#A `BlockRowCountField` — display no longer clamps to `MAX_BLOCK_ROWS_HINT` (GPT MEDIUM).** Fix #2 (first follow-up) surfaced the ACTUAL rendered row count when `blockRowsHint` is unset, but the DISPLAYED value was still clamped to the schema max of 100. A legal legacy spacing-only Method-B deck can render > 100 rows (100 ft × narrow deck + `blockSpacingMm = MIN_BLOCK_SPACING_MM = 300` → 102 rows). The field now shows the true rendered count (e.g. 102) — floor is still `MIN_BLOCK_ROWS_HINT (2)`, but the ceiling is removed for the READ-ONLY display. The schema clamp `[MIN_BLOCK_ROWS_HINT, MAX_BLOCK_ROWS_HINT]` still applies to what gets WRITTEN by user edits (via `clampCountForDispatch`) so a save/reload cannot poison the design.
> - **#B `produceAddSupportRow` derives `currentRows` via the SAME `SpanTable` path as the layout (Opus MEDIUM).** Pre-fix the remediation's `resolveMethodBGrid` call OMITTED `spanTable` (there was a `void table;` smell) — the layout used `max(legacyDefault, spanSafeRows)` while the remediation used `legacyDefault` only. Correct by coincidence today (all real IRC allowables ≥ 1220 mm), but coincidence is fragile. The `table` parameter is now threaded into the call, along with `design.joist.material` + `design.joist.spacingMm`, so both callers produce the same `currentRows` by construction. `clampMethodBRows` continues to gate the proposed count — `wouldClear` stays consistent.
> - **#C Vestigial `_legacyColsHint` param removed from `resolveMethodBGrid` (Opus LOW).** The underscore-prefixed parameter was a genuine no-op — the schema keeps `blockColsHint` for back-compat (no-op since PR #66 pinned columns to joist x-centers) but the resolver never inspected it. Signature and both call sites cleaned up.
> - **#D Fail-loud comment on the ≤ 1-block display fallback (Opus LOW).** The `MIN_BLOCK_ROWS_HINT + 1 = 3` fallback branch in `BlockRowCountField.effectiveDisplayCount` is unreachable per the perimeter-two invariant (Method B always renders ≥ 2 rows). A render-time throw is inappropriate in UI code; an in-line comment pins the invariant so a future regression is easy to spot in review.
>
> All in-PR. Method A + elevated remain byte-identical. Gate stays SEQUENTIAL.
...
- **FR-036** *(Blocking between joists — solid INTERIOR lateral restraint per IRC R502.7 / R502.7.1; added 2026-07-05 issue #72 — feat/blocking-between-joists; SUPERSEDES FR-029's "blocking MemberKind is RESERVED" clause; **wording clarified 2026-07-05 PR #73 review — GPT-5.5 HIGH #3: end restraint no longer over-claimed for Method B**)*: For EVERY design that produces ≥ 2 joists — elevated (both posts-on-footings AND deck-blocks foundations) AND floating (Method A `beams-and-joists` AND Method B `joists-on-blocks`) — `computeLayout` and `computeFloatingLayout` MUST emit `MemberKind = 'blocking'` members: SOLID lumber noggins between EVERY pair of adjacent joists at one or more INTERIOR mid-span row stations along +z. This FR provides INTERIOR bridging / anti-rotation restraint only. The joist END restraint required by IRC R502.7 is a SEPARATE concern, provided by: (a) the rim joist / ledger for elevated framing; (b) the two rim beams the layout already emits for floating **Method A** (`beams-and-joists`); and (c) — **NOT YET provided** for floating **Method B** (`joists-on-blocks`), which has no rim/band member at present. Adding a joist-depth end band for Method B is tracked as a separate story in **issue #74**. Interior bridging alone does NOT constitute full R502.7 compliance for Method B; do not claim it does. **Row rule (IRC R502.7.1 — "at intervals not exceeding 8 feet").** Row count `N = max(1, ceil(lengthMm / MAX_BLOCKING_SPACING_MM) - 1)` with `MAX_BLOCKING_SPACING_MM = 2438 mm (8 ft)` — the ceiling to the next-tighter integer pitch. The `-1` counts INTERIOR rows only (blocking never lands at `±L/2`). The `max(1, ...)` floor ensures ≥ 1 mid-span row whenever there are ≥ 2 joists — a conservative DIY choice (IRC prescribes bridging OR blocking for joist depth > 2×10 nominal, but blanket application to every joist depth is a safe superset). Rows are spread EVENLY: `z_k = -L/2 + k × L/(N+1)` for `k = 1..N` (verify: N=1 → z=0 mid-span; N=2 → z=±L/6; the pitch between adjacent rows AND between an end and its nearest row stays ≤ 2438 mm by construction for L ≤ 4876 mm, and the ceiling formula tightens N whenever L would exceed it). **Per-member geometry (in the joist plane).** `size.x` = CLEAR gap between the two joist FACES (`xCenters[i+1] − xCenters[i] − joistThickness`) — blocking sits BETWEEN the joists, not through them, so it cannot overlap either joist AABB; `size.y` = joist depth (full-depth solid blocking); `size.z` = joist thickness (blocking is the SAME 2× lumber as the joist, oriented with its thickness along +z); `position.x` = bay midpoint (`(xCenters[i] + xCenters[i+1]) / 2`); `position.y` = joistCenterY (the same y as the joist members — read from `computeYStack.joistCenterY` for elevated and `computeYStackFloating.joistCenterY` for floating so blocking cannot drift from joists under any y-stack change including the FR-034 flush-vs-drop dispatch and the FR-030 deck-blocks post derivation); `position.z` = the interior row station; `rotation` = `(0, 0, 0)`; `material` = the joist's lumber material (blocking piggybacks on the joist BOM SKU and renders emerald via the existing `MATERIAL_KIND_COLORS.blocking`); `id` = deterministic `blocking-r{row}-b{bay}`. **Wiring.** ONE shared PURE domain helper `layoutBlockingBetweenJoists({ joistXCenters, joistCenterY, joistThicknessMm, joistDepthMm, lengthMm, material })` in `src/domain/layout/blocking-layout.ts` is called from BOTH pipelines via thin per-pipeline adapters (`computeBlockingFromDesign` in `layout-engine.ts`, `computeFloatingBlockingFromDesign` in `floating/floating-layout.ts`) that derive x-centers from the SAME `computeJoistXCenters(widthMm, spacingMm, thicknessMm)` used by joist layout. The helper is pure (same input → byte-equal output — pinned by a test) and boundary-clean (NO react/three imports — `scripts/boundary-selftest.mjs` stays 28/28). **Edge cases.** Fewer than 2 joists (impossibly-narrow deck) ⇒ zero blocking members (no adjacent bay exists), no crash. Length short enough that the ceiling formula gives N < 1 ⇒ still N=1 (the floor). **Zero-width bay guard (PR #73 review GPT-5.5 HIGH #1).** `validateJoistSpacing` (`layout-shared.ts`) admits `actualSpacing == joistThickness` (adjacent joists touching face-to-face; clear gap == 0). At that exact boundary the helper MUST SKIP the degenerate bay (`clearGap ≤ CLEAR_GAP_EPS_MM = 1e-6`, matching the upstream validator's EPS) — no invisible zero-width mesh, no zero-length BOM cut. Fail-safe skip, not a throw. The row loop continues so any OPEN bay in the same row still emits blocking normally. **Barrel export (PR #73 review GPT-5.5 MEDIUM #4).** `MAX_BLOCKING_SPACING_MM` is re-exported from `src/domain/layout/index.ts` via `./blocking-layout` (= 2438 mm), NOT via `./floating/floating-layout` — the pre-#72 DEAD legacy constant of the same name in floating-layout.ts (= 1220 mm) was DELETED to eliminate the name / value collision. **BOM.** The existing per-kind length rule in `derive-bom.ts:132` (blocking length = `size.x`, runs along +x) folds blocking cuts into the joist SKU's first-fit-decreasing pack automatically; a `deriveBom(computeLayout(defaultDesign))` integration test pins the wiring per-member (asserts count parity per length, not just per-unique-length presence). **UI.** `LayerVisibility.blocking: boolean` (default `true` — safety-relevant lateral restraint is ON by default), one row `{ key: 'blocking', label: 'Blocking' }` in `LAYER_ITEMS` between "Joists" and "Beams" (physical-stack top-down order), one entry `'blocking'` in `DECK_LAYER_ORDER` between `'beams'` and `'joists'` (bottom-up render order — blocking sits in the joist plane so renders before joists in the scene graph), `<BlockingLayer />` mounted in `DeckLayers.tsx` between `<BeamsLayer />` and `<JoistsLayer />`; `BlockingLayer` delegates to `<KindLayer kind="blocking" visibilityKey="blocking" />` (identical shape to every sibling KindLayer). Toggling the checkbox hides/shows every blocking mesh via the shared visibility path. **Persistence.** NO `.deck` schema change — blocking is a derived output of `computeLayout`, not a persisted design field. Every existing `.deck` file continues to load byte-identical; the SAVED design is unchanged and the RENDERED layout adds the code-mandated noggins on load. **Regression pin.** All ~20 layout golden fixtures (elevated + floating) were REGENERATED — blocking members are appended in the emit order `[boards, joists, blocking, beams, posts, footings/blocks]` (elevated) and `[…joists, blocking, beams|blocks, …]` (floating). Fixture diffs are the EXPECTED consequence of this FR; every downstream consumer that keys off member kind (BOM packer, PlanView2D renderer, part-color palette) already handled `'blocking'` from S22 and needed no update. **Supersedes FR-029's "The additional `blocking` `MemberKind` is RESERVED — no floating (or elevated) layout path emits `blocking` members in the current MVP" clause** — blocking is now emitted by every layout path.
