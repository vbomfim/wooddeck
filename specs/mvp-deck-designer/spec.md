...
**Owner**: @vbomfim
**Last updated**: 2026-07-06 (feat/block-spacing S28 amendment — Method B rewritten as a user-controllable block grid at `blockSpacingMm` pitch; the pre-S28 "one column per joist" model is superseded; over-span remediation now reduces `blockSpacingMm`; validator tolerates IEEE-754 drift; MAX_DECK_DIMENSION_MM mirrored on both elevated + floating paths)
...
> **2026-07-06 amendment (S28 — feat/block-spacing PR #66):** User (DIY homeowner) UAT surfaced two related issues with the pre-S28 Method B ("joists on blocks") layout: (a) block count exploded (~150 blocks on a 16-ft deck) because the pre-S28 model placed a block under EVERY joist × N rows; (b) the user had NO CONTROL over "how far apart my blocks are." S28 replaces the pre-S28 Method B block placement with a user-controllable REGULAR block GRID at pitch `foundation.blockSpacingMm` (both axes), with the outer blocks anchored at the footprint edges. The S28 additions:
>
...
- **FR-029** *(Floating-deck layout engine — parallel to the elevated engine; rescoped 2026-07-05 S26 review-gate)*: When `structure = 'floating'`, the layout engine MUST produce a `Layout` that dispatches on `design.floatingFraming` (see FR-033) and MUST honor `design.joist.spacingMm` (the pre-S26 layout ignored it — that is a fixed bug). The floating layout adds ONE new `MemberKind` (`block`) beyond the pre-amendment five (`joist`, `beam`, `post`, `footing`, `board`). The additional `blocking` `MemberKind` is RESERVED — no floating (or elevated) layout path emits `blocking` members in the current MVP; the kind is preserved for a future "blocking between joists" feature and the `BlockingLayer` component remains in the tree as a dormant `() => null`. The two active floating paths are:
  ...
  - **Method B — `'joists-on-blocks'`.** Beam-less framing. The layout MUST produce: (a) N joists running along the LENGTH axis at `design.joist.spacingMm` (same helper as Method A); (b) NO beam members; (c) a REGULAR block grid at pitch `foundation.blockSpacingMm` on BOTH axes (columns across +x, rows along +z), with the outer blocks anchored at the footprint edges (`computeAxisCenters`); per-axis count = `max(2, ceil(spanMm / spacingMm) + 1)` — the perimeter-two floor + a `ceil` step that guarantees adjacent-block gap ≤ `spacingMm`; (d) decking directly on the joists. y-stack (bottom-up): block → joist → decking. **[amended 2026-07-06 — S28 feat/block-spacing]** The pre-S28 model pinned block columns to joist x-centers and used `blockRowsHint` as the S25 seam; the S28 model is a REGULAR grid at pitch `blockSpacingMm` with a deterministic block-count cap (`MAX_METHOD_B_BLOCK_COUNT = 400`, closed-form enforcement). Joists no longer necessarily sit exactly over a block column — this is an illustrative / planning-model acknowledgement; the safety-critical over-span check is UNCHANGED (span-check derives the joist support span from the block GRID's row pitch, not from per-column matches — see FR-035 for full details). `blockSpacingMm > legacy hints` precedence: when both `blockSpacingMm` and `blockRowsHint`/`blockColsHint` are set, spacing wins.
...
- **FR-035** *(Method B block spacing — user-controllable grid pitch; added 2026-07-06 S28 review-gate — feat/block-spacing PR #66)*: For `structure === 'floating' && floatingFraming === 'joists-on-blocks'` (Method B), the block layout MUST be a REGULAR grid at user-selectable pitch `foundation.blockSpacingMm: Mm` on BOTH axes (columns across width +x, rows along length +z), with the outer blocks anchored at the footprint edges (`computeAxisCenters`). Per-axis count MUST be `blockCountForAxis(spanMm, spacingMm) = max(2, ceil(spanMm / spacingMm) + 1)` — the perimeter-two floor + a `ceil` step that guarantees adjacent-block gap ≤ `spacingMm`. `blockSpacingMm` is OPTIONAL on both block-carrying `FoundationSpec` variants (`deck-blocks` + `tuffblocks`); when ABSENT the layout MUST default to `DEFAULT_METHOD_B_BLOCK_SPACING_MM = 1220 mm (4 ft)` — a sensible ground-level DIY pitch chosen to keep the default joist span within IRC allowables for the MVP joist catalog. When PRESENT the value MUST be clamped to `[MIN_BLOCK_SPACING_MM = 300 mm, MAX_BLOCK_SPACING_MM = 2438.4 mm = 8 ft]` at BOTH the layout boundary AND the UI input boundary (so the stored value is always schema-legal — save/reload cannot trap the user in an invalid state). Total block count MUST be bounded by `MAX_METHOD_B_BLOCK_COUNT = 400` via a DETERMINISTIC closed-form enforcement in `resolveMethodBGrid`: when the un-capped requested counts satisfy `colsReq × rowsReq > MAX_METHOD_B_BLOCK_COUNT`, compute the aspect `a = colsReq / rowsReq` and derive `capRows = max(2, min(rowsReq, floor(sqrt(MAX / a))))`, `capCols = max(2, min(colsReq, floor(capRows × a)))` — aspect-preserving so the grid stays visually proportional to the footprint AND the perimeter-two floor holds. Floor-arithmetic rounding CAN leave a tiny headroom where `capCols × capRows > MAX_METHOD_B_BLOCK_COUNT` after clamp; a single tail-shrink on the LARGER axis (`capCols -= 1` if `capCols ≥ capRows > 2`, else `capRows -= 1`) is guaranteed sufficient by construction. A postcondition assertion (`throw LayoutError` if `cols × rows > MAX_METHOD_B_BLOCK_COUNT`) MUST guard the return so any future arithmetic bug surfaces immediately rather than hiding behind a silently-too-dense grid. No iteration cap — the enforcement is O(1). Defense-in-depth: `.deck` schema v2 `Dimensions3D.widthMm` / `lengthMm` MUST carry `maximum: 30480 mm (100 ft)` (a residential cap; the shared `Mm` `$def` allows 1,000,000 mm which is oversized for this app), AND `MAX_DECK_DIMENSION_MM` MUST be mirrored at BOTH the elevated `validateDesign` and the floating `validateFloatingDesign` trust boundaries so an in-memory patch / test-fixture path (no Ajv round-trip) hits the same guard. When `blockSpacingMm` is present the resolver PREFERS it over the legacy `blockRowsHint` / `blockColsHint` hints (which remain in the schema for backwards-compat with pre-S28 fixtures but are LEGACY-SUPERSEDED for Method B). Joists still run along +z at `design.joist.spacingMm`; the "joists sit directly over a block column" invariant is NO LONGER guaranteed under Method B (illustrative planning-model — the DIY user can nudge on paper). Span-check for Method B is UNCHANGED in shape: `span-check.ts::deriveMethodBJoistSpanFromBlockGrid` derives the joist support span from the block GRID row pitch (max adjacent-z gap between block rows), so an `over-span-joist` warning fires when the user picks a `blockSpacingMm` too large for the joist material and clears when they pick one small enough. Every joist MUST be span-checked regardless of whether its x falls under a block column (no silent skip). The Method B `'add-support-row'` remediation from FR-032 MUST reduce `blockSpacingMm` rather than bump `blockRowsHint` (see FR-032 Method B bullet). Persistence: `blockSpacingMm` is an OPTIONAL number in `.deck` schema v2 with `minimum: 300`, `maximum: 2438.4`, `additionalProperties: false` preserved. Missing on load → the layout applies the default at compute time; existing pre-S28 `.deck` files continue to load unchanged. The UI MUST expose a `BlockSpacingField` (LengthField idiom — imperial/metric via `formatLength` / `parseLength`) VISIBLE only when `structure === 'floating' && floatingFraming === 'joists-on-blocks'`; commits MUST clamp `[MIN, MAX]` BEFORE dispatch. `MIN_BLOCK_SPACING_MM` / `MAX_BLOCK_SPACING_MM` / `DEFAULT_METHOD_B_BLOCK_SPACING_MM` MUST be re-exported through the `state` barrel (the dep-cruiser `ui-no-domain-layout` rule forbids the UI from importing `src/domain/layout/**` directly). **Float-drift tolerance.** `computeAxisCenters` accumulates `-span/2 + i × step` (`step = span / (count − 1)`); IEEE-754 mul-add rounding can drive the LAST center a few femtometres OUTSIDE `±span/2` for fractional (schema-legal) footprints (e.g. `span=10205.753894382558, count=19 → last=5102.876947191281` vs bound `5102.876947191279`, Δ ≈ 1.8e-12 mm). `validateExplicitCenters` MUST accept centers within `±span/2 ± EPS_MM` where `EPS_MM = 1e-6 mm` (matching `validateJoistSpacing`'s tolerance in `layout-shared.ts`) so schema-legal fractional footprints do not crash on drift. The tolerance is a nanometre — a real callsite bug (millimetres out of range or worse — the S26 defense's target) is still rejected. **Regression pin.** Method A + elevated + posts-on-footings layouts MUST be BYTE-IDENTICAL to pre-S28 (`blockSpacingMm` alters ONLY Method B; the validator tolerance changes no block positions); every pre-S28 golden fixture stays unchanged.
